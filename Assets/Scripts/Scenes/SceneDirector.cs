...
public class SceneDirector : MonoBehaviour
{
    ...
    public void ResetStuff()
    {
        ...
        {
            if (npc.characterController == null)
            {
                throw new Exception($"{npc.name} has no charcter controller");
    ...
    public async Task PlayScene(string[] outputLines, List<AudioClip> voiceActingClips)
    {
        ...
        foreach (string line in outputLines)
        {
            ...
            if (line.Contains('[') && !line.Contains(":"))
            {
                ...
                if (lowerLine.Contains("walks to ") || lowerLine.Contains("walk to "))
                {
                    if (character2 != null)
                    {
    ...
    private async Task<int> PlayAudioClipAtIndex(List<AudioClip> voiceActingClips, int audioClipIndex)
    {
        ...
        {
            Debug.Log($"Playing voice track #{audioClipIndex}");
